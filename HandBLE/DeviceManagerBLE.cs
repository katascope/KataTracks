...
namespace HandBLE
{
    ...
    public class DeviceManagerBLE
    {
        ...
        public static void StartMonitoring(List<string> bleDeviceConnectionList)
        {
            ...
            foreach (string str in bleDeviceConnectionList)
            {
        ...
        public static async void QueryGATT(BluetoothDevice bd)
        {
            ...
            device.log += " Found: " + gattServices.Count + "\n";
        ...
        static private void MonitorDevice(object in_name)
        {
            ...
            while (!bleDevices.ContainsKey(id) || bleDevices[id].stayActive)
            {
                if (bleDevices.ContainsKey(id) && bleDevices[id].serviceCache.Count == 0)
                {
                    ...
                    BluetoothDevice bd = Task.Run(() => BluetoothDevice.FromIdAsync(id)).Result;

                    while (bd == null)
                        bd = Task.Run(() => BluetoothDevice.FromIdAsync(id)).Result;

                    if (bd != null && !bleDevices[id].queryingGatt)
                    {
                        Console.WriteLine("GattQuery : " + id + " " + bleDevices[id].queryingGatt);
                        Task.Run(() => QueryGATT(bd)).Wait();
                    }
                }
                else
                {
